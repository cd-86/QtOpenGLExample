...
LightingMapsExample::LightingMapsExample(QWidget *parent) :
...
void LightingMapsExample::timerEvent(QTimerEvent *event)
{
...
void LightingMapsExample::enterEvent(QEvent *event)
{
    // 隐藏鼠标指针，将指针置于窗口中心
...
void LightingMapsExample::leaveEvent(QEvent *event)
{
}

void LightingMapsExample::mouseMoveEvent(QMouseEvent *event)
{
...
void LightingMapsExample::wheelEvent(QWheelEvent *event)
{
...
void LightingMapsExample::keyPressEvent(QKeyEvent *event)
{
    switch(event->key()) {
    case Qt::Key_W:
        keys.W = true;
...
void LightingMapsExample::keyReleaseEvent(QKeyEvent *event)
{
    switch(event->key()) {
    case Qt::Key_W:
        keys.W = false;
...
void LightingMapsExample::initializeGL()
{
    ...
    lightingShader.addCacheableShaderFromSourceFile(QOpenGLShader::Vertex, "shaders/lighting_maps/lighting_maps.vs");
    lightingShader.addCacheableShaderFromSourceFile(QOpenGLShader::Fragment, "shaders/lighting_maps/lighting_maps.fs");
    if(!lightingShader.link()) {
        qDebug() << lightingShader.log();
    };
    lightCubeShader.addCacheableShaderFromSourceFile(QOpenGLShader::Vertex, "shaders/lighting_maps/light_cube.vs");
    lightCubeShader.addCacheableShaderFromSourceFile(QOpenGLShader::Fragment, "shaders/lighting_maps/light_cube.fs");
...
void LightingMapsExample::resizeGL(int w, int h)
{
    QOpenGLFunctions *f = context()->functions();
...
void LightingMapsExample::paintGL()
{
    QOpenGLFunctions* f = context()->functions();
